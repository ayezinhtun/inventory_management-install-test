...
export default function AdminRelocationRequests() {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      // Admin sees items awaiting admin approval (from PM-approved) and history
      const data = await getRelocationRequests(["pm_approved", "admin_approved", "rejected", "complete"]);
      setRequests(data || []);
    } catch (e) {
  ...
  const pendingCount = requests.filter(r => r.status === 'pm_approved').length;
          ...
          <Table hoverable>
            <TableHead>
              <TableRow>
                ...
                <TableHeadCell>{requests.some(r => r.status === 'pm_approved') ? 'Action' : ''}</TableHeadCell>
              </TableRow>
            </TableHead>
                    ...
                    <TableRow key={r.id} className="bg-white">
                      ...
                      <TableCell className="flex item-center space-x-3">
                        {r.status === 'pm_approved' && (
                          <>
                            <button
                              onClick={() => handleStatusChange(r.id, "admin_approved")}
