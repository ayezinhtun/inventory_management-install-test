...
export default function PhysicalRelocationRequests() {
  ...
  const handleComplete = async (id) => {
    ...
    try {
      ...
      setToast({ type: "error", message: "Failed to complete request." });
    }
  };

  const totalCount = requests.length;
  const pendingCount = requests.filter(r => r.status === 'admin_approved').length;

  return (
    <div>
      <h1 className="font-bold mb-5 text-[24px]">Relocation Requests — Physical Execution</h1>
      ...
      <div className="grid grid-cols-3 gap-y-5 gap-10 mb-5">
        ...
        <CardComponent title="To Complete" count={pendingCount} icon={Clock} color="bg-yellow-100" iconColor="text-yellow-600" />
      </div>
