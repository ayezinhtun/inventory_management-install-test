...
export default function Header() {
  ...
  const handleLogout = async () => {
    setOpen(false); setLoggingOut(true);
    try { await logOut(); navigate('./login'); }
    catch (err) { console.error(err); setLoggingOut(false); }
  };
      ...
      <div className="flex items-center gap-2">
        <Link to='/notification'>
          <button type="button" className="relative flex" key={unreadCount}>
            ...
            />
            {unreadCount > 0 && (
              <span className="absolute -top-3 -right-1 inline-flex items-center justify-center rounded-full bg-red-600 text-white text-xs min-w-[18px] h-[18px] px-1">
                {unreadCount}
              </span>
            )}
          </button>
          ...
          {open && (
            <div className='absolute -right-5 mt-4 w-48 border border-gray-200 rounded-lg shadow-lg bg-white z-50'>
              ...
              <div className='flex flex-col py-1'>
                <button onClick={() => closeAndNavigate('/setting')} className='flex items-center gap-2 px-4 py-2 hover:text-[#26599F] text-gray-700 text-left'>
